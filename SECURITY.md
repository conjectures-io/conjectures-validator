...
| Read host files, mutate trusted metadata, or contact another process | Landlock ABI 4+, verifier-tree-only reads, workspace-only content writes, safe `/dev` nodes, no `/proc`, and seccomp denial of metadata mutation, sockets, `io_uring`, pidfds, cross-process memory/limits/signals, kernel keyrings, IPC, and namespace/mount syscalls |
