...
REWARD_FAMILY_CONSTRAINT = "submissions_reward_family_reward_unique"
...
class NewSubmission:
    ...
    reward_family_id: str  # stable across source repins and related statements
    task_mode: TaskMode
...
async def create_submission(
    ...
    submission = Submission(
        ...
        reward_family_id=request.reward_family_id,
...
async def reward_family_holder(
    session: AsyncSession, submission: Submission
) -> uuid.UUID | None:
    """The other submission already holding this reward family's single reward, if any.

    Mirrors the predicate of `submissions_reward_family_reward_unique`. The index is the authority;
    this only lets the service give the reason instead of surfacing an IntegrityError.
    """
    result = await session.execute(
        select(Submission.id)
        .where(
            Submission.reward_family_id == submission.reward_family_id,
...
async def approve_automatically(
    ...
    * another submission already holds the reward. That is not a fault in this proof, but it
    ...
    holder = await reward_family_holder(session, submission)
    if holder is not None:
        superseded = ReviewDecision(
            ...
            notes=f"reward family {submission.reward_family_id} is already held by {holder}",
        )
