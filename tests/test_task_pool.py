...
def test_task_selection_is_new_and_audited_across_source_families():
    ...
    assert len({item.source_path for item in selected}) == 113
    assert all(
...
def test_newly_retired_targets_are_recorded_but_not_admitted():
    newly_retired = {
        ...
        "Green29.green_29",
        "Green42.green_42",
        # 2026-08-12: one exact direct problem was solved before submissions arrived, and one
        # paid formalization-defect review required the affected task to be quarantined.
