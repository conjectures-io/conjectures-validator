...
def test_workspace_is_fresh_and_packages_only_one_submission(tmp_path):
    ...
    first = create_workspace(task_dir=task, project_root=project, workspace_parent=tmp_path / "work")
    second = create_workspace(task_dir=task, project_root=project, workspace_parent=tmp_path / "work")
    try:
        assert first.root != second.root
        submission = Submission(tmp_path / "Main.lean", "theorem target : True := by trivial\n", b"", "sha256:x")
        package_solution(first, task, submission)
