...
The current pool contains 100 audited Erdős tasks from 67 source files. It
contains no Written on the Wall II task. The GitHub review covered all 281 open
...
allowlist. `retired-source-theorems.json` prevents either previous pool from
being selected again.

## Rebuilding

`scripts/rebuild_gold_pool.py` loads the exact checked-in selection audit,
builds every target, and inspects the resulting pool. It refuses to overwrite
an existing task directory or allowlist. Generate into fresh staging paths,
review the selection and hashes, and only then replace the published pool.
