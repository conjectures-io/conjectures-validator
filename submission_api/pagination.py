...
# `created_at` and a UUID id. Competition feeds do not share it -- each competition lives in its
# own database with its own keys, and its adapter decides the keyset position of a row. Another
# dataclass per shape would be another copy of the signing, the base64 and the five ways a
# cursor can be malformed, which is the part worth having exactly once.
#
...
def decode_parts(
    secret: str, value: str, *, version: str, count: int | None
) -> tuple[str, ...]:
    ...
    has nothing to learn from being told which of the checks it failed. `count=None` accepts
    any non-zero number of parts, for a caller whose shape is chosen by someone else and which
    validates the parts itself.
    """
    if not value or len(value) > MAX_CURSOR_LENGTH:
        raise _invalid()
    payload_b64, _, signature = value.partition(".")
    ...
    expected = len(fields) - 1 if count is None else count
    if len(fields) < 2 or len(fields) != expected + 1 or fields[0] != version:
        raise _invalid()
