...
Four ways in. Three are for a browser and end in an HttpOnly cookie: Google, a magic link to an
email address, and a signature from a coldkey. The fourth is for the miner CLI and ends in a
bearer token: a signature from a hotkey already linked to an account in the browser. See
`submission_api/sessions.py` for the two credential forms and why only cookies need CSRF, and
