...
r=pathlib.Path('/tmp/add-50-20260908');p=pathlib.Path('/tmp/conjectures-validator-a8d559d-erdos564');c=load_catalog(pathlib.Path('/root/conjectures-validator/data/catalog.json'));ds={x.theorem:x for x in c.declarations};rows=json.loads((r/'overlap-replacement.json').read_text());out=[]
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
 for x in pool.map(job,[(x['theorem'],m) for x in rows for m in ['formalized','counterexample']]):
  out.append(x);(r/'generation-overlap-replacement.json').write_text(json.dumps(out,indent=2)); print(len(out),x['status'],x['theorem'],x['mode'],x.get('error',''),flush=True)
