#!/usr/bin/env python3
"""Broad, reproducible cheap-tactic sweep over all 100 staged candidate tasks."""
...
AUDIT_ROOT = Path("/tmp/add-50-20260908/attack-audit-overlap")
...
def load_targets() -> tuple[Any, list[dict[str, Any]]]:
    if (
        subprocess.check_output(
            ("git", "-C", str(VALIDATOR_ROOT), "rev-parse", "HEAD"), text=True
        ).strip()
        != VALIDATOR_COMMIT
    ):
        raise RuntimeError("validator baseline commit drifted")
    catalog = load_catalog(VALIDATOR_ROOT / "data/catalog.json")
    if catalog.repository_commit != FORMAL_CONJECTURES_COMMIT:
        raise RuntimeError("catalog source pin drifted")
    root = Path("/tmp/add-50-20260908")
    names = [row["theorem"] for row in json.loads((root / "overlap-replacement.json").read_text())]
    ...
    if len(targets) != 2 or len({item["task_id"] for item in targets}) != 2:
...
def main() -> int:
    ...
    summary = {
        ...
        "baseline": {
            ...
            "staging_summary": "/tmp/add-50-20260908/generation-final.json",
            "replacement_staging": "/tmp/add-50-20260908/bundles",
