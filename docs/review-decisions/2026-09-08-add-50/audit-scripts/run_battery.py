...
AUDIT_ROOT = Path("/tmp/add-50-20260908/attack-audit")
...
def load_targets() -> tuple[Any, list[dict[str, Any]]]:
    ...
    names = [row["theorem"] for row in json.loads((root / "selection.json").read_text())]
    ...
    if len(targets) != 100 or len({item["task_id"] for item in targets}) != 100:
