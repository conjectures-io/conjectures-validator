...
r=pathlib.Path('/audit');p=pathlib.Path('/opt/fc-verifier');c=load_catalog(pathlib.Path('/release/data/catalog.json'));ds={x.theorem:x for x in c.declarations};rows=json.loads((r/'new-targets.json').read_text());out=[]
