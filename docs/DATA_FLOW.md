...
flowchart TD
    subgraph GEN["GENERATION DOMAIN — offline, trusted, once per pin"]
        ...
        PICK["task target policy<br/>137 active asserted picks"]
        ...
        ALLOW["conjectures-tasks/allowlist.json<br/>274 bundle digests · default DENY"]
    end
...
| 7 | active after retiring targets a verified submission settled | **137** targets from 116 files (118 Erdős over 100 files, 19 Green over 16) |
...
The 137 active targets are **not computed** from the 506. They are the admitted subset of 146
...
137 `allowed_source_theorems` and 274 `allowed_task_bundles`. Each bundle entry pins `task_id`,
...
all 118 active Erdős targets and 19 Green targets.
