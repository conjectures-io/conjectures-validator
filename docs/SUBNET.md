...
- Related parents, parts, variants, and proof/refutation modes share one stable reward family and
  may produce at most one reward.
...
**One reward per Erdős family is a constraint, not a convention.** The pool issues a `formalized`
and a `counterexample` task for each theorem target. `submissions` carries both its source-pinned
`problem_id` and stable `reward_family_id`, derived from the allowlist at intake so a miner cannot
choose the payout identity. `submissions_reward_family_reward_unique` is a unique index on
`reward_family_id` over every row whose `reward_status` is not `INELIGIBLE`. Parent statements,
parts, variants, proof/refutation pairs, and later source repins therefore share one possible
reward. `FAILED` is inside that predicate deliberately: a payout that failed keeps its claim and
is retried on the same row.
...
- an audited allowlist of 148 proof/counterexample bundles for 74 Erdős theorem targets in 55
  stable reward families;
...
- automatic reward eligibility and cross-target one-reward-per-family enforcement.
...
6. Add scoring-policy versions, weight batches, Subnet chain submission, and reconciliation.
   Automatic eligibility and one-reward-per-family enforcement are implemented.
